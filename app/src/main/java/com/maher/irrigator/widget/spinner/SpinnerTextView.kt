...
class SpinnerTextView<DataType> : AppCompatTextView {
    ...
    private var mHintTextEnable: Boolean = true
    ...
    fun getSelectedItem(): DataType? {
        return if (mItemSelectedPosition != -1) {
            mAdapter.getRealItem(mItemSelectedPosition)
        } else {
            return null
        }
    }
    ...
    private fun init(context: Context?, attrRes: AttributeSet?) {
        ...
        context?.obtainStyledAttributes(attrRes, R.styleable.SpinnerTextView)?.run {
            mDropDownItemHeight = getLayoutDimension(
                R.styleable.SpinnerTextView_dropdown_item_height,
                WindowManager.LayoutParams.WRAP_CONTENT
            )
            mDropDownListMaxHeight = getDimensionPixelSize(
                R.styleable.SpinnerTextView_dropdown_max_height,
                WindowManager.LayoutParams.WRAP_CONTENT
            )
    ...
    private fun calculatePopupWindowHeight(): Int {
        ...
        var listViewHeight = 10 * itemHeight
    ...
    fun notifyReset() {
        ...
        text = context.getString(R.string.select_a_plant)
        mHintTextEnable = true
        mAdapter.enableHintText(true)
        isEnabled = true
        recalculatePopupWindowHeight()
    }
}