            ...
            with(spinnerTextView.getSelectedItem()) {
                this?.let {
                    if (spinnerTextView.isEnabled && count.length() > 0 && flow.length() > 0) {
                        SavedData.saveDataObject(context, LPlant(it.dev, it.init, it.late, it.mid, this.name, count.text.toString().toDouble(), calendar.get(Calendar.DAY_OF_YEAR), flow.text.toString().toDouble()), lTag)
